...
<template>
    ...
    <div class="w-full h-[200px] flex flex-col">
      <div class="flex w-full justify-between h-full">
        <div class="ml-[114px] h-[184px] shrink-0 relative">
          <Image
            src="https://web-res-ccc.afunimg8.com/cdn-cgi/image/format=auto/C02/vip/l/vip1_l.png" alt=""
            importance="auto" class="!h-auto md:!h-full"
          />
        </div>
        <div class="flex flex-col justify-between pt-[12px] pb-[28px] flex-1 max-w-[800px] ml-auto mr-[5px]">
            ...
            <div>
              <h4 class="text-[--bc-color20] flex justify-between items-centertext-[20px] mb-[4px] font-bold">
                ...
                <div class="flex items-center overflow-hidden h-full cursor-pointer text-color hover:text-white">
                  <i
                    class="inline-block h-[max-content] w-[max-content] icon-new-live-support cursor-pointer text-[18px] mt-[1px] text-active"
                  />
          ...
          <div class="text-color flex items-center justify-between text-[14px]">
            <div>
              {{ (userInfo?.userVip?.nextCoin || 0) - (userInfo?.userVip?.vipCoin || 0) }}XP to VIP {{
                (userInfo?.userVip?.vipLevel || 0) + 1 }}
              <i
                class="inline-block h-[max-content] w-[max-content] icon-new-info-normal cursor-pointer ml-[10px] relative hover:text-white"
              >
                <div
                  class="opacity-0 hidden select-none pointer-events-none invisible absolute w-[400px] left-0 top-[26px] p-[10px] pr-[14px] box-shadow-02 border-radius-0 bg-[--bc-bgColor9] z-[200]"
                >
  ...
  <BaseModal v-if="isPc" v-model:show="show" content-class="w-[700px]">
    ...
    <div class="text-[14px] flex-1 bg-color-3 overflow-hidden border-radius-0 flex flex-col">
      <div
        class="shrink-0 flex text-justify items-center h-[46px] px-[20px] text-[--bc-color20] font-semibold vip-level-item"
      >
      ...
      <div class="flex-1 overflow-y-auto overflow-hidden">
        <div
          ...
          class="flex items-center px-[20px] h-[46px] text-[--bc-textColor] vip-level-item bg-linear-12"
        >
          <div class="flex-1 shrink-0 flex items-center">
            <Image :src="item.icon" alt="" importance="auto" class="shrink-0 mr-[6px] !h-auto" />
            <span>{{ item.name }}</span>
          </div>
          <div class="flex-1 shrink-0 px-[4px] text-center">
